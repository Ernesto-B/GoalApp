...
function SmartGoalExplanation() {
  return (
    <Card className="border-2 border-dashed border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/30">
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center">
          <Lightbulb className="h-5 w-5 mr-2 text-amber-500" />
          SMART Goal Framework
        </CardTitle>
        <CardDescription>Design effective goals with these principles</CardDescription>
      </CardHeader>
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        <div className="flex items-center">
                          {blueprint.icon}
                          <h3 className="font-medium ml-2">{blueprint.title} Blueprint</h3>
                          {blueprint.isCustom && (
                            <span className="text-xs ml-2 px-1.5 py-0.5 bg-purple-100 dark:bg-purple-900/40 text-purple-800 dark:text-purple-300 rounded">Custom</span>
                          )}
                        </div>
                          ...
                          <div className="flex items-start">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">
                                This will create {blueprint.goals.length} new goals based on this blueprint. Each goal will have appropriate deadlines set automatically.
                              </p>
                            </div>
                          </div>
                        </div>
...
function DeadlineGuidance({ selectedType, selectedDate }: { selectedType: string, selectedDate: Date | null }) {
  ...
  // Calculate active goals in that timeframe
  const getActiveGoalsCount = () => {
    if (!goals || !selectedDate) return 0;
    
    return goals.filter(goal => {
      // Only count non-completed, non-archived goals
      if (goal.isCompleted || goal.isArchived) return false;
      
      const deadline = new Date(goal.deadline);
      const now = new Date();
      
      // Goal is active if the deadline is in the future and overlaps with the selected date
      return isAfter(deadline, now) && isAfter(deadline, selectedDate);
    }).length;
  };
  
  // Get appropriate timeframe recommendations
  const getTimeframeRecommendations = () => {
    const now = new Date();
    ...
    switch(selectedType) {
      case "short":
        return {
          min: addDays(now, 7),
          ideal: addDays(now, 14),
          max: addDays(now, 30)
        };
      case "medium":
        return {
          min: addDays(now, 30),
          ideal: addMonths(now, 2), 
          max: addMonths(now, 3)
        };
      case "long":
        return {
          min: addMonths(now, 3),
          ideal: addMonths(now, 6),
          max: addMonths(now, 12)
        };
      default:
        return {
          min: addDays(now, 7),
  ...
  const timeframes = getTimeframeRecommendations();
  const activeGoalsCount = getActiveGoalsCount();
  
  let recommendation = "Looks good!";
  let recommendationColor = "text-green-500";
  
  if (selectedDate) {
    const daysFromNow = differenceInDays(selectedDate, new Date());
    
    if (daysFromNow < differenceInDays(timeframes.min, new Date())) {
      recommendation = "Consider extending your deadline for a more realistic timeframe.";
      recommendationColor = "text-amber-500";
    } else if (daysFromNow > differenceInDays(timeframes.max, new Date())) {
      recommendation = "This is quite far in the future. Consider breaking this down into smaller goals.";
      recommendationColor = "text-amber-500";
    }
    
    if (activeGoalsCount > 5) {
      recommendation = "You have many active goals in this timeframe. Consider prioritizing or sequencing them.";
  ...
  return (
    <Card className="border-2 border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/30">
      <CardHeader className="pb-2">
        ...
        <CardDescription>Set realistic timeframes for success</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="flex items-center justify-between text-sm">
          <span>Recommended timeframe:</span>
          <span className="font-medium">
            {selectedType === "short" ? "7-30 days" : 
             selectedType === "medium" ? "1-3 months" : 
             "3-12 months"}
          </span>
        </div>
        
        <div className="flex items-center justify-between text-sm">
          <span>Ideal deadline:</span>
          <span className="font-medium">
            {format(timeframes.ideal, "MMM d, yyyy")}
          </span>
        </div>
        
        <div className="flex items-center justify-between text-sm">
          <span>Active goals in timeframe:</span>
          <span className={cn(
            "font-medium",
            activeGoalsCount > 5 ? "text-orange-500" :
            activeGoalsCount > 3 ? "text-amber-500" : 
            "text-green-500"
          )}>
            {activeGoalsCount}
          </span>
        ...
        <div className="p-3 rounded-md bg-white dark:bg-gray-800 mt-3">
          <div className="flex items-start">
            <InfoIcon className={cn("h-5 w-5 mr-2 flex-shrink-0", recommendationColor)} />
            <p className={cn("text-xs", recommendationColor)}>{recommendation}</p>
          </div>
        </div>
      </CardContent>
    </Card>
